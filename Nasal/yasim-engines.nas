...
var Jet =
{
	...
	new: func(n, running = 0, idle_throttle = 0.01, max_start_n1 = 5.21, start_threshold = 3, spool_time = 4, start_time = 0.02, shutdown_time = 4,) 
	{
		...
		m.fuelboost = props.globals.getNode("controls/switches/pilot/rapid-takeoff-panel/fuel-boost[" ~ n ~ "]", 1);
	...
	update: func {
		...
		{
			var rpm = me.rpm.getValue();
			if ( ( rpm >= 20 ) or  ( ( rpm >= me.start_threshold ) and ( ( me.fuelboost.getValue() ) or ( me.number == 2 ) ) ) )
			{
				var time_delta = getprop("sim/time/delta-realtime-sec");
				rpm += time_delta * me.spool_time;
				me.rpm.setValue(rpm);
...
setprop("/systems/APU/throttle", 0.8);
